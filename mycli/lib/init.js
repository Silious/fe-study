...
async function initProject(projectName) {
  try {
    ...
    if (exists) {
      ...
      const files = await fse.readdir(path.resolve(__dirname, "../template"));
      console.log(files);
      const selectedTem = await inquirer.prompt([
        {
          name: "template",
          message: "选择模板",
          type: "list",
      ...
      if (!exists) {
        ...
        console.log(
          symbols.error,
          chalk.red(`Copy template failed. 模板不存在...`)
        );
      }
